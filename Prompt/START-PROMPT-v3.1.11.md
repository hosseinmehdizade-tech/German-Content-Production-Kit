...
8. Source/dictionary sense markers such as `[1]`, `[1a]`, `[2b]` are evidence locators only. Normalize them according to `SOURCE-SENSE-MARKER-NORMALIZATION-v1.0.0.md`: remove only the marker from learner-facing text, preserve the lexical item unchanged, retain the locator in provenance when available, and never delete a synonym/antonym/collocation solely because the marker existed. Marker-only repair must preserve relation counts.
