...
8. If a headword explicitly encodes valency, emit explicit Rektion with evidence; do not guess ambiguous two-way-preposition case.
9. Inventory the source once, establish stable canonical IDs, cache external source responses, and retry only missing/stale/failed units with backoff. A repeated full-source refetch is a pipeline defect.
10. For a new source with no dedicated completeness profile, use `GERMAN-RICH-CARD-CONTENT-COMPLETENESS-v1.1.0.json` as the default quality/coverage policy. `MENSCHEN-A1-CONTENT-COMPLETENESS-v1.1.0.json` applies only to Menschen A1.
11. Resolve the intended Flashcards Pro runtime at delivery time. v354 is the current verified baseline, but do not keep using v354 if the target application has advanced.
12. Before `Final`, execute canonical/architecture validation, linguistic audit, lexical-quality validation, product coverage/completeness reporting, delivery projection, version-pinned target runtime/import+presentation acceptance, packaging, and post-package hash verification.
13. A file that merely parses, or a transport-only PASS, is not a runtime-verified release.
14. Continue agentically through all applicable stages without arbitrary batch stops. Stop only for a real evidence/tooling blocker, report exactly what is blocked, and preserve completed checkpoints so the run can resume without repeating finished work.
15. Final delivery must include the direct import TSV separately in addition to the archive/package and QA reports.
